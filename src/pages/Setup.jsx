...
export default function Setup() {
  ...
  return (
    <div className="min-h-screen bg-th-bg flex flex-col items-center justify-center px-4 py-12">
      <div className="w-full max-w-lg">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="text-3xl font-extrabold text-th-text tracking-tight">World Cup 2026 Pool</h1>
          <p className="text-th-muted text-sm mt-2">One-time setup — only the pool organiser does this</p>
        </div>
        ...
        <div className="mb-4">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sm font-semibold text-th-text">Choose Storage Mode</span>
          </div>
              ...
              <button
                ...
                className={cn(
                  ...
                  mode === id
                    ...
                    : 'border-th-border bg-th-border/40 hover:border-th-muted'
                )}
              >
                <Icon className={cn('h-5 w-5', mode === id ? 'text-[#FFD706]' : 'text-th-muted')} />
                <div>
                  <div className={cn('font-bold text-sm', mode === id ? 'text-[#FFD706]' : 'text-th-text')}>{title}</div>
                  <div className="text-xs text-th-muted mt-0.5">{sub}</div>
                </div>
              </button>
            ))}
          </div>
          ...
          <Card className="mb-4">
            <CardHeader className="pb-3">
              ...
              <CardDescription className="text-xs space-y-1">
                <p>Pool name and admin password are set in the server's <code className="bg-th-border px-1 rounded">.env</code> file — no credentials are stored in the browser.</p>
                <p className="mt-1">Make sure the server is running (<code className="bg-th-border px-1 rounded">npm run server</code>) before continuing.</p>
              </CardDescription>
            </CardHeader>
          </Card>
        )}
        ...
        {mode === 'local' && (
          <Card className="mb-4">
            ...
            <CardContent className="space-y-3">
              ...
              <div>
                <Label>Admin Password <span className="text-th-muted font-normal">(only you need this)</span></Label>
